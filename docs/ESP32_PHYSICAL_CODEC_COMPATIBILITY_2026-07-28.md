...
- Fixed firmware: commits `b73623f` and `ca186cb`.
...
| DivX 3, 320x180 Big Buck Bunny | **PASS, now opens** | 11.994 / 12 | 1.541 / 5.959 | 46.744 / 69.448 | 30.194 / 31.047 | 78.478 / 100.565 | 0/120 | PCM audio clean; 120 consecutive frames, no decode error or frame gap. |
| DivX 3, 320x240 Big Buck Bunny | **PASS, now opens** | 12.012 / 12 | 1.557 / 5.179 | 54.399 / 80.074 | 40.911 / 41.821 | 96.867 / 122.686 | 0/120 | PCM audio clean; 120 consecutive frames, no decode error or frame gap. |
...
The DivX 3 fix replaces the complete compressed-packet allocation with a
decoder-owned 4096-byte refill buffer. Packet size is still bounded and known
from AVI, but memory use no longer grows with the largest packet. A second fix
allows a short direct VLC entry at the very end of a packet even when there
are not enough remaining bits to form the longer escape code. The host
regression decoded all 7157 frames of both Big Buck Bunny variants and
compared the streaming and contiguous decoded-frame checksums; packets up to
35,161 bytes crossed the 4096-byte refill buffer without a mismatch.
...
- HLV 320x240 and MPEG-1 320x240 with MP2 audio still have internal-RAM
  limitations.
