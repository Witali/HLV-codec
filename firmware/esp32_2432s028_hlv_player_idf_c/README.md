...
ramp and uses six exact-cycle DMA buffers. After the ramp, all buffers contain
the same full-scale waveform and repeat without task-side refills. SD, display,
SPI, Wi-Fi and the internal DAC remain disabled.
...
it. The browser lists regular `.hlv`, `.bpv1`, `.avi`, `.mpg`,
...
Use an input whose `max_packet` is greater than `input_buffer`. The `J`
record reports both values plus `refills` and `refill_bytes`; its RGB565 hash
must equal the contiguous control.
...
callback averages, followed by input-buffer/refill metrics. The callback average measures RGB565 hashing and is
