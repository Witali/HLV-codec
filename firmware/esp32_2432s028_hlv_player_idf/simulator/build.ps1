...
$commandTemplate = 'call "{0}" -no_logo -arch={1} && cd /d "{2}" && ' +
    ...
    '/DHLV1_FAST_32BIT_BITREADER={5} /DHLV1_LEVEL_LOOKAHEAD={6} ' +
    '/I"{7}" /I"{8}" "{9}" "{10}" "{11}" /Fe:"{12}"'
$command = $commandTemplate -f $devcmd, $Architecture, $build,
    $optimizationFlag, $DecoderStats, $bitReaderFlag, $LevelLookahead,
    $include, $privateInclude, $simulator, $common, $decoder, $output
