...
// This capacity is deliberately unrelated to the maximum JPEG packet.
constexpr size_t kMjpegInputRingBytes = 8 * 1024;
constexpr size_t kMjpegInputRingStorageBytes = kMjpegInputRingBytes + 1;
constexpr size_t kMjpegInputChunkBytes = 512;
...
constexpr size_t kMjpegInputSpeculativeBytes =
    kMjpegInputRingBytes - kMjpegInputChunkBytes;
constexpr uint32_t kMjpegInputReaderStackBytes = 3072;
constexpr uint32_t kMjpegInputStopTimeoutMs = 500;
constexpr uint32_t kMjpegInputWaitTimeoutMs = 3000;
constexpr size_t kMjpegMaximumPacketBytes = 1024 * 1024;
constexpr uint32_t kMjpegInputCommandDecode = 1;
...
#if MJPEG_INPUT_PREFETCH
struct MjpegInputPacket {
    int result = MJPEG_AVI_OK;
    size_t jpeg_size = 0;
};
#endif
...
#if MJPEG_INPUT_PREFETCH
StreamBufferHandle_t mjpeg_input_stream = nullptr;
StaticStreamBuffer_t mjpeg_input_stream_state{};
alignas(4) uint8_t
    mjpeg_input_stream_storage[kMjpegInputRingStorageBytes]{};
QueueHandle_t mjpeg_input_packet_queue = nullptr;
StaticQueue_t mjpeg_input_packet_queue_state{};
uint8_t mjpeg_input_packet_queue_storage[sizeof(MjpegInputPacket)]{};
TaskHandle_t mjpeg_input_reader_task_handle = nullptr;
StaticTask_t mjpeg_input_reader_task_state{};
...
size_t readDivx3File(void *context, uint8_t *buffer, size_t capacity) {
    ...
    for (;;) {
        ...
        if (request.codec == VideoCodec::kMpeg1) {
            ...
            Divx3ReplayReader stream{
                readDivx3File, request.divx3_file, 0, 0};
...
BpvInputStartResult startBpvInputPrefetch() {
    if (!video_file || bpv_input_stream ||
        bpv_input_reader_task_handle) {
        return BpvInputStartResult::kInvalidState;
    }
    bpv_input_stream =
    ...
    if (!bpv_input_stream || !bpv_input_chunk) {
        ...
        return BpvInputStartResult::kNoMemory;
    }
    ...
    if (!prefetched &&
        (bpv_input_reader_done || millisNow() >= deadline)) {
        stopBpvInputPrefetch();
        return BpvInputStartResult::kReadFailed;
    }
    ESP_LOGI(
        kTag,
    ...
    return BpvInputStartResult::kReady;
}

#if MJPEG_INPUT_PREFETCH
int mapAviDemuxToMjpeg(int result) {
    switch (result) {
...
bool sendMjpegInputPacket(const MjpegInputPacket &packet) {
...
bool sendMjpegInputBytes(const uint8_t *bytes, size_t size) {
    size_t sent = 0;
...
void mjpegInputReaderTask(void *) {
    std::array<uint8_t, kMjpegInputChunkBytes> chunk{};
    AviDemuxInfo demux_info{};
    ...
    while (!mjpeg_input_stop_requested) {
        AviDemuxPacket avi_packet{};
        MjpegInputPacket packet{};
        uint32_t command = 0;
        ...
        size_t prefetch_remaining =
            std::min(remaining, kMjpegInputSpeculativeBytes);
        while (prefetch_remaining && !mjpeg_input_stop_requested) {
            const size_t bytes =
                std::min(prefetch_remaining, chunk.size());
            if (std::fread(chunk.data(), 1, bytes, video_file) != bytes ||
                !sendMjpegInputBytes(chunk.data(), bytes)) {
                remaining = SIZE_MAX;
                break;
            }
            remaining -= bytes;
        ...
        while (!mjpeg_input_stop_requested &&
               xTaskNotifyWait(0, UINT32_MAX, &command,
                               pdMS_TO_TICKS(20)) != pdTRUE) {
        }
        ...
        while (remaining && !mjpeg_input_stop_requested) {
            const size_t bytes = std::min(remaining, chunk.size());
            if (std::fread(chunk.data(), 1, bytes, video_file) != bytes ||
                !sendMjpegInputBytes(chunk.data(), bytes)) {
                remaining = SIZE_MAX;
                break;
...
bool signalMjpegInputPacket(bool decode) {
    if (!mjpeg_input_reader_task_handle) return false;
...
void stopMjpegInputPrefetch() {
    const int64_t deadline = millisNow() + kMjpegInputStopTimeoutMs;
    ...
    while (mjpeg_input_reader_task_handle &&
           !mjpeg_input_reader_finished && millisNow() < deadline) {
        vTaskDelay(pdMS_TO_TICKS(1));
    }
    if (mjpeg_input_reader_task_handle) {
        vTaskDelete(mjpeg_input_reader_task_handle);
        mjpeg_input_reader_task_handle = nullptr;
    }
    if (mjpeg_input_stream) {
        vStreamBufferDelete(mjpeg_input_stream);
        mjpeg_input_stream = nullptr;
    }
    if (mjpeg_input_packet_queue) {
        vQueueDelete(mjpeg_input_packet_queue);
        mjpeg_input_packet_queue = nullptr;
    }
    mjpeg_input_reader_finished = true;
}

int nextMjpegInputPacket(MjpegAviPacket *packet) {
    MjpegInputPacket input_packet{};
    if (!packet || !mjpeg_input_packet_queue ||
        xQueueReceive(mjpeg_input_packet_queue, &input_packet,
                      pdMS_TO_TICKS(kMjpegInputWaitTimeoutMs)) != pdTRUE) {
        return MJPEG_AVI_ERR_IO;
    }
    if (input_packet.result != MJPEG_AVI_OK) return input_packet.result;
    *packet = {};
...
#else
bool startMjpegInputPrefetch() { return true; }
void stopMjpegInputPrefetch() {}
#endif
    ...
    if (!sd_dma_aligned_buffer) {
        ...
        if (!sd_dma_aligned_buffer) {
            ESP_LOGE(kTag, "Cannot reserve %u-byte SD DMA buffer",
                     static_cast<unsigned>(kSdDmaMinimumBlockBytes));
            return false;
        }
    }
...
int prefetchImaAudioBlock(size_t block_bytes) {
    ...
    if (block_bytes < sizeof header ||
        std::fread(header, 1, sizeof header, audio_file) != sizeof header ||
    ...
    first_sample[0] = static_cast<uint8_t>(state.predictor);
    first_sample[1] = static_cast<uint8_t>(
        static_cast<uint16_t>(static_cast<int16_t>(state.predictor)) >> 8);
    queueAudioBytes(first_sample, sizeof first_sample);
    remaining_samples = static_cast<size_t>(sample_count) - 1U;
    remaining_bytes = block_bytes - sizeof header;
    while (remaining_bytes) {
        const size_t chunk = std::min(remaining_bytes, sizeof compressed);
        size_t output = 0;
        if (std::fread(compressed, 1, chunk, audio_file) != chunk)
            return HLV1_ERR_IO;
        for (size_t input = 0;
             input < chunk && remaining_samples; ++input) {
            int16_t decoded = ima_adpcm_decode_nibble(
                &state, compressed[input] & 15U);
            audio_read_chunk[output++] = static_cast<uint8_t>(decoded);
            audio_read_chunk[output++] = static_cast<uint8_t>(
                static_cast<uint16_t>(decoded) >> 8);
            --remaining_samples;
...
int prefetchImaWavAudioBlock(size_t block_bytes) {
    ...
    while (remaining_bytes) {
        ...
        if (std::fread(compressed, 1, chunk, audio_file) != chunk)
            return HLV1_ERR_IO;
        for (size_t input = 0; input < chunk; ++input) {
            int16_t decoded = ima_adpcm_decode_nibble(
                &state, compressed[input] & 15U);
            audio_read_chunk[output++] = static_cast<uint8_t>(decoded);
            audio_read_chunk[output++] = static_cast<uint8_t>(
...
bool onAudioPdmSent(i2s_chan_handle_t handle,
    ...
    if (dma_slot < kAudioDmaDescriptors) {
        audio_dma_valid_samples[dma_slot] =
            static_cast<uint16_t>(received / sample_bytes);
        audio_pending_samples =
            audio_pending_samples +
            static_cast<uint32_t>(received / sample_bytes);
    }
...
bool prepareAudio(const HLV1Header &header) {
    ...
    if (dma_free < kAudioDmaMinimumFreeBytes ||
        ...
        ESP_LOGW(
            ...
            static_cast<unsigned>(kAudioDmaMinimumFreeBytes),
            static_cast<unsigned>(kAudioDmaBufferBytes));
        stopAudio();
        return false;
    }
    ...
    const uint32_t queued_ms = static_cast<uint32_t>(
        static_cast<uint64_t>(kAudioStreamBytes) * 1000U /
...
bool isPacketVideoMemoryError(int result) {
...
void showVideoOpenFailure(const char *invalid_title, const char *detail,
                          bool out_of_memory) {
...
bool openVideo() {
    ...
    if (video_codec == VideoCodec::kMpeg1) {
        ...
        if (result != H263_3GP_OK) {
            ...
            showVideoOpenFailure(
                video_codec == VideoCodec::kMpeg4Simple
                    ? "Invalid MPEG-4 SP"
                    : "Invalid H.263",
        ...
        if (bpv_header.version >= BPV1_PIXEL_MOTION_VERSION) {
            ...
            if (input_result != BpvInputStartResult::kReady) {
                const bool out_of_memory =
                    input_result == BpvInputStartResult::kNoMemory;
                showVideoOpenFailure(
...
bool renderMpegBRows(
    ...
    if (player_settings::kScaleVideoToDisplay) {
        for (int output_y = 0; output_y < kScreenHeight; ++output_y) {
            ...
            if (source_y < static_cast<int>(first_y) ||
                source_y >= static_cast<int>(first_y + row_count)) continue;
            uint16_t *pixels = display.acquireBuffer();
            if (!pixels) return false;
            convertMpegRow(
                frame, source_y - static_cast<int>(first_y), true, 0,
                pixels, kScreenWidth);
            if (display.drawBitmap(
                    0, output_y, kScreenWidth, 1,
                    pixels) != ESP_OK) return false;
        }
        return true;
    }
    ...
    const int absolute_begin = static_cast<int>(first_y) > source_y
        ? static_cast<int>(first_y) : source_y;
    const int absolute_end = std::min(
        static_cast<int>(first_y + row_count), visible_end_y);
    const int x_offset = (kScreenWidth - width) / 2;
    const int y_offset = (kScreenHeight - height) / 2;
    const int rows_per_transfer = display.rowsPerTransfer();
    for (int absolute_y = absolute_begin; absolute_y < absolute_end;) {
        ...
        for (int row = 0; row < rows; ++row) {
            convertMpegRow(
                frame, absolute_y + row - static_cast<int>(first_y),
                false, source_x, pixels + row * width, width);
        }
        if (display.drawBitmap(
                x_offset, y_offset + absolute_y - source_y,
                width, rows, pixels) != ESP_OK) return false;
        absolute_y += rows;
    }
    return true;
}
...
void failSdCardRead(const char *detail) {
    ...
    const size_t dma_largest =
        heap_caps_get_largest_free_block(MALLOC_CAP_DMA);
    if (!sd_dma_aligned_buffer && dma_largest < kSdDmaMinimumBlockBytes) {
        ESP_LOGE(
            kTag,
            "SD read could not allocate DMA memory: free=%u largest=%u: %s",
            static_cast<unsigned>(dma_free),
            static_cast<unsigned>(dma_largest), detail);
...
PresentationState mpeg_b_presentation{};
bool mpeg_b_presentation_active = false;
bool mpeg_b_render_ok = true;
uint32_t mpeg_b_render_us = 0;
uint32_t mpeg_b_callback_us = 0;
...
void consumeMpegBFrameRows(
    ...
    if (mpeg_b_presentation.render && mpeg_b_render_ok) {
        ...
        mpeg_b_render_us += static_cast<uint32_t>(
            microsNow() - render_start);
    }
    mpeg_b_callback_us += static_cast<uint32_t>(
        microsNow() - callback_start);
}
...
void applyKeyframeCatchupWithLookahead(
    ...
    if (state.late && !keyframe && next_keyframe) {
        // BPV exposes the next packet's keyframe flag before decode. Prefer
        // spending the remaining display time on that I-frame.
        state.render = false;
    ...
    if (state.late) {
        if (keyframe) {
            ...
            if (late >=
                    player_settings::kKeyframeCatchupLateFrames) {
                video_waiting_for_keyframe = true;
            }
        }
    } else {
...
void playOneMpegFrameStreamedRows() {
    ...
    if (decoded.mpeg_frame.storage_mode ==
        ...
        const uint32_t decode_us = decoded.decode_us - std::min(
...
void playOneDivx3Frame() {
    ...
    Divx3ReplayReader stream{readDivx3File, video_file, 0, 0};
    if (video_waiting_for_keyframe) {
        uint8_t prefix = 0;
        int intra = 0;
        const int64_t skip_start = microsNow();
        if (std::fread(&prefix, 1, 1, video_file) == 1) {
            stream.prefix = prefix;
            stream.prefix_pending = 1;
            if (divx3_packet_probe_intra(&prefix, 1, &intra) == DIVX3_OK &&
                ...
                finishCompressedPredictiveSkip(
                    read_us,
                    static_cast<uint32_t>(microsNow() - skip_start));
                return;
            }
        }
    }
...
void playOneBpvFramePipelined() {
    ...
    if (pending_bpv_frame_valid && packet.info.keyframe &&
        ...
        rendered = presentBpvFrameWithLookahead(
