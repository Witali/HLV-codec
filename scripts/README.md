...
| `reencode_production_avi_audio_ima.ps1` | Applies the checked single-file converter to every production H.263 and MPEG-4 SP AVI, mapping Bunny, Danila, and the regression clip back to their original audio sources. Use `-ListOnly` to validate the complete mapping without changing media. |
| `reencode_custom_audio_ima.ps1` | Replaces only the audio tails of one or more HLV/BPV files with normalized mono IMA ADPCM, normally at 32 kHz. It prepares a shared source once, validates every packet and IMA block, and requires matching SHA-256 hashes of the compressed video stream before replacing a file. |
| `encode_mpeg1.ps1` | Encodes constrained MPEG-1 Program Stream with no B pictures and MP2 mono 32 kHz audio from an explicitly signed PCM16 source. Controls include dimensions, `VideoQuality`, GOP, audio bitrate, and frame limit. |
| `encode_h263_avi.ps1` | Encodes baseline H.263 only at standard QCIF `176x144` or CIF `352x288`, always in AVI and at the full source frame rate, with optional normalized mono WAV IMA ADPCM at the source rate. Constant-quality Q6 is the default; use `VideoQuality=1..31` to override it or pass zero for bitrate control. |
| `encode_mpeg4_simple_avi.ps1` | Encodes bounded MPEG-4 Part 2 Simple Profile at `320x240` in M4S2 AVI, with I/P pictures only, full source rate up to 30 fps, and optional normalized mono WAV IMA ADPCM at the source rate. The default Q5 profile is the accepted `35dB` quality profile. `-Preset Esp32Speed` is an explicit lower-quality decoder-speed trade-off that uses zero/integer motion vectors and stronger coefficient elimination. |
...
all four AVI profiles use standard AVI IMA ADPCM at the source rate up to
48 kHz; MPEG-1 uses MP2 at 32 kHz. Inputs without
...
| `test_avi_demux.ps1` | Builds the shared C99 AVI demultiplexer test and scans every AVI under `out`, validating all video/audio packet boundaries including nested `LIST rec` chunks. |
| `test_mpeg1_audio_pcm16.ps1` | Encodes a deterministic 32 kHz tone into MP2 from signed PCM16, builds the embedded mono MP2 synthesis path, and verifies signed 16-bit output with detail below the old U8 quantization step. |
