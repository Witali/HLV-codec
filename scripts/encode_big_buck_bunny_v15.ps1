...
if (-not $OutputFile) {
    $OutputFile = Join-Path $repo `
        ("out\HLV\BigBuckBunny_1080p_video-settings_v15_{0}_level-curve.hlv" -f
            $fpsLabel)
}
...
$temporaryAudio = Join-Path ([IO.Path]::GetTempPath()) `
    ("hlv-bbb-v15-{0}.u8" -f [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $profileMessage = (
        "Encoding 320x180 at {0}, stable HLV v15, quality 45, " +
        "GOP 30, threads {1}..."
