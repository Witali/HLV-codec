...
# H.263 source decoding is built into both project players. New project assets
# use only standard baseline H.263 QCIF/CIF in AVI.
...
Assert-FfmpegCapability -Arguments @("-hide_banner", "-demuxers") `
    ...
    -Description "the MOV/MP4/3GP source demuxer"
Assert-FfmpegCapability -Arguments @("-hide_banner", "-muxers") `
    -Pattern '^\s*E\s+avi(?:\s|$)' `
    -Description "the AVI muxer used for standard H.263"
Assert-FfmpegCapability -Arguments @("-hide_banner", "-demuxers") `
    -Pattern '^\s*D\s+avi(?:\s|$)' `
    -Description "the AVI demuxer used for standard H.263"
...
Write-Host (
    "The baseline H.263 encoder and AVI mux/demux support are available."
)
